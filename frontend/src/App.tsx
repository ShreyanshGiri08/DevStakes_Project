...
import { Orbit, LogOut, Download, Clock } from 'lucide-react';
import { useRef, useCallback } from 'react';

export default function App() {
  const { userEmail, userDisplayName, userPhotoUrl, activeNodes, activeDeepDiveNodeId, currentTopic, logout, availableTime, setAvailableTime } = useStore();
  ...
  if (activeNodes.length === 0) return <LandingScreen />;
      ...
      >
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="hidden md:flex items-center gap-2 px-3 py-1.5 bg-slate-800/60 rounded-lg border border-slate-700/50">
            ...
            <input type="range" min={15} max={120} step={15} value={availableTime}
          ...
          <button onClick={handleExportPDF}
            className="p-2 rounded-lg bg-slate-800/60 hover:bg-slate-700 border border-slate-700/50 transition-colors" title="Export PDF">
